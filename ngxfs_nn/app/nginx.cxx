...
{   
    ...
    for(i = 0; environ[i]; i++) // environment var
    {
    ...
    if(p_config->Load("nginx.conf") == false)  // load config to m_ConfigItemList
    {   
    ...
    if(ngx_init_signals() != 0)  //initialize signal // setup signal handler
    {
        exitcode = 1;
        goto lblexit;
    }        
